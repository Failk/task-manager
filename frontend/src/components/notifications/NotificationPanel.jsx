...
import { FiX, FiBell, FiCheck, FiClock } from 'react-icons/fi';
...
const NotificationPanel = ({ isOpen, onClose }) => {
  ...
  const handleSnooze = async (notification) => {
    try {
      ...
      toast.success('Notification snoozed for 15 minutes');
      loadNotifications();
    } catch (error) {
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-xl z-50 flex flex-col">
        ...
        <div className="flex gap-2 p-4 border-b">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm ${
              filter === 'UNREAD' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700'
            }`}
          >
            Unread
          </button>
                      ...
                      {!notification.read && (
                        <div className="flex gap-2 mt-3">
                          <button
                            ...
                            className="text-xs px-3 py-1 bg-primary-100 text-primary-700 rounded-lg hover:bg-primary-200 flex items-center gap-1"
                          >
                            <FiCheck size={12} />
                            Mark as read
                          </button>
                          <button
                            ...
                            className="text-xs px-3 py-1 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 flex items-center gap-1"
                          >
                            <FiClock size={12} />
                            Snooze 15m
                          </button>
