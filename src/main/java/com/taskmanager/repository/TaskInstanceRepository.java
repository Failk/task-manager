...
public interface TaskInstanceRepository extends JpaRepository<TaskInstance, Long> {
    ...
    @Query("SELECT ti FROM TaskInstance ti WHERE ti.recurringTask.user.id = :userId AND ti.scheduledDate = :date ORDER BY ti.scheduledTime ASC")
    List<TaskInstance> findByUserIdAndScheduledDate(@Param("userId") Long userId, @Param("date") LocalDate date);
    
    @Query("SELECT ti FROM TaskInstance ti WHERE ti.recurringTask.user.id = :userId AND ti.scheduledDate BETWEEN :startDate AND :endDate ORDER BY ti.scheduledDate ASC")
